...
const samehadaku = {
  ...
  getLinkVideo: async function (url) {
    try {
      ...
      for (let s of servers) {
        ...
        const ajaxResp = await got.post(`${this.base}/wp-admin/admin-ajax.php`, {
          headers: {
        ...
        console.log(`📥 Response length for ${s.label}:`, ajaxResp.body.length);

        const match = ajaxResp.body.match(/<iframe[^>]+src="([^"]+)"/);
        const streamUrl = match ? match[1] : null;
        console.log(`🔗 Stream URL for ${s.label}:`, streamUrl);

        streamResults.push({ quality: s.label, url: streamUrl });
      }
  ...
  search: async function (page = 1, query = '') {
    try {
      ...
      $('article.animpost').each((i, el) => {
        ...
        const genres = [];
        $(el).find('.stooltip .genres a').each((j, g) => { genres.push($(g).text().trim()); });
  ...
  getTopAnime: async function () {
    try {
      const html = await got(this.base, {
        headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" }
      }).text();
  ...
  detail: async function (url) {
    try {
      ...
      if (!description) description = $(".entry-content.entry-content-single p").text().trim();
