...
async function editAnyUser({ adminKey, targetEmail, updates = {} }) {
  ...
  // --- PROFILE, BANNER, ROLE, APIKEY, dll ---
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "expired")) {
    ...
    if (e === null) {
      ...
      // jumlah hari
      const baseTs = (user.expired && Number(user.expired) > now) ? Number(user.expired) : now;
      const newTs = baseTs + Math.round(e) * 24 * 60 * 60 * 1000;
      updateDoc.expired = Math.max(newTs, now);

    } else if (typeof e === "string") {
      const n = Number(e); // konversi string ke number
      if (!Number.isNaN(n)) {
        const baseTs = (user.expired && Number(user.expired) > now) ? Number(user.expired) : now;
        const newTs = baseTs + Math.round(n) * 24 * 60 * 60 * 1000;
        updateDoc.expired = Math.max(newTs, now);
      } else {
        throw new Error("expired string tidak bisa di-parse");
      }

    } else {
      throw new Error("updates.expired harus null | number(hari) | string angka(hari)");
    }
  }
...
async function checkEmailAndDevice({ email, deviceId }) {
  if (!email || !deviceId) throw new Error("Email dan deviceId wajib diisi");
  const _db = await connectDB();
  const users = _db.collection("users");
  const emailExists = await users.findOne({ email });
  if (emailExists) throw new Error("Email sudah terdaftar");
  const deviceExists = await users.findOne({ deviceId });
  if (deviceExists) throw new Error("Device ID sudah terdaftar");
  return true;
}
...
async function minLimit(apikey) {
  ...
  let updateDoc = {};
  let decrementLimit = 0;

  // Downgrade premium yang expired
  ...
  if (["user", "premium"].includes(user.role)) {
    ...
    } else if (!updateDoc.limit) {
      decrementLimit = Math.min(1, user.limit);
    }
  }

  let updateQuery = {};
  if (Object.keys(updateDoc).length) updateQuery.$set = updateDoc;
  if (decrementLimit > 0) updateQuery.$inc = { limit: -decrementLimit };
  ...
  const finalLimit = Math.max(resultUser.limit || 0, 0);
