...
async function registerUser({ email, password, deviceId }) {
  ...
  const apikey = generateRandomKey(16);   // generate sekali saat register
  const now = Date.now();
  ...
  const user = {
    ...
    banner: null,
    role: "user",
    apikey,               // simpan key di user
    createdAt: now,
  };

  await users.insertOne(user);

  // simpan juga ke collection "keys"
  const keyData = {
    key: apikey,
    limit: 100,
    initlimit: 100,
  ...
  await saveKeyToMongo(apikey, keyData);
